	...
	for year in years:
		...
		for filepath in filelist:
			with open(filepath,'rU') as csvfile:
				...
				print csvreader.next() # pop off header
				for line in csvreader:
					try:
						if line:
							data.append([airport,line[0],line[1],line[2], \
								line[3], line[4], line[5], line[7], line[9], line[14]])
...
ap, time, temp, hum, pressure, v, date = zip(*data)
temp=np.asarray(temp,dtype='float')
date=[el.strip('<br />') for el in date] #get rid of some nonsense

import pandas
import matplotlib.pyplot as plt
dpan=pandas.to_datetime(date)
